...
(
  Select user_id,
  DATEDIFF(s,time_stamp, LEAD(time_stamp) OVER (PARTITION BY user_id ORDER BY time_stamp)) as next_stamp
  From Confirmations
) as table_temp
Where next_stamp <= 24*60*60

/* Bets query LeetCode resolve problem */
with cte as (
    select
    user_id,
    case when DATEDIFF(second, lag(time_stamp) over 
        (partition by user_id order by time_stamp), time_stamp)
        <= 86400
        then 1
        else 0 end toConsider
...
-- select DATEDIFF(second,'2021-06-12 11:57:29','2021-06-13 11:57:30')